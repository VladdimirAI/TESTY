...
class MapUserRepositoryTest { // по факту проверка самого хранилища - чтоно сохраняет и достает результаты - в данном репозитории хранилище это мапа а в настоящем может быть же ибд - носкорее всего это не будет являться интеграционным тестом
    ...
    //названиеТестируемогоМетода_чтоДолженДелать_ИкогдаОнДолженЭтоДелать
    @Test
    @DisplayName("Проверяет что что положили то и получаем")
    ...
    @Test
    @DisplayName("Проверяет что из репозитория достаеться именно последний юзер - если оба были засейваны под одни id")
